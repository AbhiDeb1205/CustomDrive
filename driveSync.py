...
class GCloudDriver:
    ...
    def upload_file(self, file_path, mime_type, folder_id=None):
        ...
        if self.md_data is not None and file_path in self.md_data.keys():
            # If curr filename present in drive
            
            for idx, upfile in enumerate(self.md_data[file_path]):
                print("upfile:",upfile)
...
###FIX-ME:: ADD check if successfully updated by comparing md5
                        self.md_data[file_path][idx]["md5"] = fmd5
                        self.md_data[file_path][idx]["modified_time"] = formatted_modified_date
